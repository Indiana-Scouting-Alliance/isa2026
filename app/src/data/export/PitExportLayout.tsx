...
export default function PitExportLayout({
  showPublicApiToken,
        ...
        {robotColumnsState.length > 0 && (
          <>
            <p className={styles.columnsHeader}>
              Select columns to include in pit scouting data
            </p>
            <div className={styles.columnsContainer}>
              {PitScoutEntryColumns.map((column, columnIndex) => {
                return (
                  <Checkbox
                        ...
                        {
                          {
                            ...
                            string:
                              column === "startingLocation" ?
                                <DataTypeIcon dataType='"trenchDepot" | "bumpDepot" | "hub" | "bumpOutpost" | "trenchOutpost"' />
                              : column === "pitClimbLocation" ?
                                <DataTypeIcon dataType='"depot" | "center" | "outpost" | "other"' />
                              : column === "pitTelePrimary" ?
                                <DataTypeIcon dataType='"score" | "feed" | "climb" | "defense" | "pickupHerd"' />
                              : <DataTypeIcon dataType="string" />,
                            number:
                              column === "pitClimbLevel" ?
                                <DataTypeIcon dataType="1 | 2 | 3" />
                              : column === "pitClimbTime" || column === "pitShooterBps" ?
                                <DataTypeIcon dataType="number" />
                          ...
                          }[typeof PitScoutEntryInit[column]]
                        }
                      </div>
                    }
                  />
...
function DataTypeIcon({ dataType }: DataTypeIconProps) {
  switch (dataType) {
    ...
    case "number": {
      return (
        <Tooltip content={<p className={styles.tooltipLabel}>number</p>}>
          <Numbers />
        </Tooltip>
      );
    }
